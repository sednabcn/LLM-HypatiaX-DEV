...
try:
    ...
    response = client.messages.create(
        model="claude-haiku-4-5-20251001",  # cheapest/fastest model for key validation
        max_tokens=10,
    ...
    if "credit balance" in str(e).lower():
        ...
        print(f"   Error: {e}")
        print("   The experiment run will fail when it tries to call the API.")
        sys.exit(2)
    else:
    ...
    print(f"❌ Unexpected error during key validation: {type(e).__name__}: {e}")
    sys.exit(1)
