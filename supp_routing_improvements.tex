...
\caption{Baseline performance before routing improvements (standard cases,
         ...
         previously-reported \textbf{89.2\%} figure is now disclosed in the
         main paper (Table~\ref{tab:main_results}\ in
...
\caption{Measured and projected $R^2 > 0.99$ (\%) by fix.
         ...
         v3.0 benchmark's uncorrected figure is \textbf{89.2\%}; the
         main paper's decision-attribution-bug correction
         (Table~\ref{tab:main_results} in \texttt{jmlr\_paper\_main.tex})
...
\midrule
\textbf{v3.0 (uncorrected)} & All fixes + benchmark corrections & --- & \textbf{89.2} & \textbf{measured} \\
\textbf{v3.0 (bug-corrected)} & Decision-attribution bug removed & $-28.4$ & \textbf{60.8} & \textbf{measured} \\
\bottomrule
\end{tabular}
\end{table}
...
Bug'').} The $89.2\%$ figure reported for the $n=74$ v3.0 benchmark was
...
LLM by 27pp" framing based on $89.2\%$ should not be cited; see the main
...
\begin{enumerate}[leftmargin=*]
  ...
  \item \textbf{[Open, process/credibility, not confirmable from
    material available to this document.]} The legacy exp2 baseline-lock
    threshold fix ($R^2 \geq 0.999999$ replacing $R^2 \geq 0.9999$, which
    had inflated the legacy baseline from 74/90 to 81/90) is applied in
    code but not yet confirmed against a live CI run of the patched
    script. Treat 74/90 as correct; do not cite 81/90 as current.
\end{enumerate}
...
figure of \textbf{89.2\%} near-perfect success rate ($R^2>0.99$, $n=74$,
...
\item Run DeFi benchmark --- confirm the corrected 60.8\% figure (not the
  superseded uncorrected 89.2\%), and that it sits below Pure LLM's
