...
Extrapolation-Reliable Analytical Discovery} (\texttt{jmlr_paper_main.tex}).
It provides full implementation details for the six routing fixes summarised in Section~\ref{sec:routing} (\emph{Component 3: Five-Stage Routing and Ensembling}) of the main paper---which now includes Proposition~1 (Routing Cascade Convergence), a formal guarantee that Phase~2 warm-starting is monotonically non-degrading over the Pareto front---and reports cumulative impact on the DeFi
...
document (Section~\ref{sec:benchmark}) overlap with equations in the Core~15
