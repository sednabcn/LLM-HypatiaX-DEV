...
def main():
    ...
    for mname, row_name in METHOD_TO_ROW.items():
        ...
        err_pcts_all = [r.get("extrap_error_pct") for r in attempted]
        err_pcts_finite = [finite(v) for v in err_pcts_all]
        n_nonfinite = sum(1 for v, f in zip(err_pcts_all, err_pcts_finite)
        ...
        print(f"{mname:45s} {tr_stats['n']:8d} "
              ...
              f"{len(attempted):9d} {n_nonfinite:19d} "
              f"{('%.1f' % err_stats['median']) if err_stats['median'] is not None else '---':>12} "
              f"{('%.1f' % err_stats['mean']) if err_stats['mean'] is not None else '---':>12}")
