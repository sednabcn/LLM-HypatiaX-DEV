...
Step IDs (use with --only / --from):
    ...
    Phase 1 : exp1  exp1b  exp2_feynman  exp2  exp3  exp3b
...
Changelog v6.0 (2026-05-07):
    EXP2-ALIGN: Removed exp2_sym and exp2_hyb steps entirely — they do not exist
               in run_all.sh. exp2_feynman and exp2 now exactly mirror run_all.sh
               using run_comparative_suite_benchmark_v2.py --benchmark feynman/all30.
...
STEPS: list[Step] = [
    ...
    # ── exp2_feynman: Feynman-30 SR via run_comparative_suite_benchmark_v2 ──────
    # Aligned with run_all.sh --step exp2_feynman.  Calls
    #   run_comparative_suite_benchmark_v2.py --benchmark feynman
    # so all six comparison methods run through the same protocol interface,
    # making output directly comparable with exp2 (all30) results.
    # Produces per-equation JSON + stats.json/results.csv/table.tex/report.html.
    # Checkpoint: logs/feynman_exp2_checkpoint.json
    #
    # Run individually:
    #   python3 hypatiax/experiments/benchmarks/run_comparative_suite_benchmark_v2.py \
    #     --benchmark feynman --checkpoint-name feynman_exp2_checkpoint --resume
    # One-equation smoke test (--one-equation):
    #   N_FEYNMAN_TASKS=1 python3 ... (same command above)
    Step("exp2_feynman",
    ...
    # ── exp2: mirrors run_all.sh --step exp2 ─────────────────────────────────
    # run_comparative_suite_benchmark_v2.py --benchmark all30
    # Run individually:
    #   python3 hypatiax/experiments/benchmarks/run_comparative_suite_benchmark_v2.py \
    #     --benchmark all30 --checkpoint-name exp2_checkpoint --resume
        ...
        for _cp_path in [_root_cp, CHECKPOINT]:
            if _cp_path.exists():
                try:
                    ...
                    pass
        # exp2 checkpoint migration: ensure key is present
