...
\newtheorem{complexity}{Complexity Result}
\providecommand{\qed}{\hfill\BlackBox}

\jmlrheading{--}{2026}{1--18}{--}{--}{--}{Supplementary Material A}

\ShortHeadings{Supplementary Material A: Routing Improvements}
              {Routing Improvements for Hybrid LLM--Neural-Network Systems}
...
\author{\name Technical Note --- Hybrid System Development Log \email{} \\
       \addr February--March 2026}
\maketitle
...
It provides full implementation details for the six routing fixes summarised in the main paper's \emph{Component 3: Five-Stage Routing and Ensembling} subsection---which now includes Proposition~1 (Routing Cascade Convergence), a formal guarantee that Phase~2 warm-starting is monotonically non-degrading over the Pareto front---and reports cumulative impact on the DeFi
...
document (the main paper's \emph{Benchmark Design} section) overlap with equations in the Core~15
...
This caveat is disclosed here; no equivalently-numbered remark exists in
the current main paper draft, so it should not be cited as appearing there.
\end{remark}
...
\caption{Baseline performance before routing improvements (standard cases,
         intractable cases excluded). Denominator $= 66$ standard cases
         (pre-v3.0 historical). The v3.0 benchmark uses $n=74$; the
         previously-reported \textbf{[VALUE REDACTED]} figure is now disclosed in the
         main paper (its primary results table, in
         \texttt{jmlr\_paper\_main.tex}, \S~``The Hybrid
         Decision-Attribution Bug'') as \emph{uncorrected} for a
         success-attribution bug. \textbf{[VALUE REDACTED --- pending re-verification against the pipeline's actual output; see reconciliation notes]}
         See that section before citing either number as current.}
\label{tab:baseline}
\begin{tabular}{lcccc}
\toprule
...
\caption{Measured and projected $R^2 > 0.99$ (\%) by fix.
         Denominator $= 66$ standard cases (pre-v3.0 historical). The
         v3.0 benchmark's uncorrected figure is \textbf{[VALUE REDACTED]}; the
         main paper's decision-attribution-bug correction
         (its primary results table, in \texttt{jmlr\_paper\_main.tex})
         gives \textbf{[VALUE REDACTED]} (provisional pending a
         file-provenance ambiguity --- see main paper) as the current
         figure \textbf{[VALUE REDACTED --- pending re-verification against the pipeline's actual output; see reconciliation notes]}. See the
         corrected-observation note below the table.}
\label{tab:projected}
\begin{tabular}{llccc}
\toprule
...
\midrule
\textbf{v3.0 (uncorrected)} & All fixes + benchmark corrections & --- & --- & --- \\
\textbf{v3.0 (bug-corrected)} & Decision-attribution bug removed & --- & --- & --- \\
\bottomrule
\multicolumn{5}{l}{\footnotesize\textsuperscript{$\ddagger$}Values redacted pending re-verification against the pipeline's actual output.}\\
\end{tabular}
\end{table}
...
\textbf{Correction (see main paper, \S~``The Hybrid Decision-Attribution
Bug'').} The \textbf{[VALUE REDACTED]} figure reported for the $n=74$ v3.0 benchmark was
subsequently found to be inflated by a success-attribution bug: a
sub-method (\texttt{hybrid.decision}) is in some cases credited for a
near-perfect success on tasks the pipeline's own records show that
sub-method had actually failed. \textbf{[VALUE REDACTED --- pending re-verification against the pipeline's actual output; see reconciliation notes]} This does not
invalidate Fixes~0--5 individually (each addresses a distinct, verified
routing or data-generation defect), but the headline "Hybrid beats Pure
LLM" framing based on \textbf{[VALUE REDACTED]} should not be cited; see the main
paper for the current, corrected comparison and its own caveats.
\end{observation}
...
\begin{enumerate}[leftmargin=*]
  ...
  \item \textbf{[Open, process/credibility.]} The July~9 CI dashboard
    should not be cited: its own notebooks (NB-01--NB-06) report ``not
    found'' for that run, while its summary asserts ``0 open issues''
    and specific claims resolved. \texttt{build\_report.py} should
    hard-fail rather than emit a stale green summary over missing
    notebook output; re-run for real before citing this dashboard again.
  \item \textbf{[Resolved.]} The legacy exp2 baseline-lock
    threshold fix ($R^2 \geq 0.999999$ replacing $R^2 \geq 0.9999$, which
    had inflated the legacy baseline from 74/90 to 81/90) is now
    live-confirmed at 74/90 against a fresh CI run of the patched script,
    and reproduced independently a second time via a patched Gate~C
    schema check. Cite 74/90; do not cite 81/90.
  \item \textbf{[Open, superseded.]} The $1.73\times$ speedup and related
    scalability/parallelisation figures in \S\,\ref{app:complexity}
    (\emph{Empirical Timing Breakdown} onward) do not reproduce against
    the mechanically-regenerated 5-seed$\times$74-task$\times$2-split
    runtime sweep (main paper \S\,10.5, ``Runtime Analysis''), which instead shows
    the hybrid system $2.5$--$9.3\times$ \emph{slower} than the neural
    baseline on every seed and split. See the superseded notice at the
    start of that subsection; do not cite its absolute timings or
    speedup figures.
\end{enumerate}
...
LLM's $R^2 > 0.99$ rate of $83.9\%$. The v3.0 benchmark's uncorrected
figure of \textbf{[VALUE REDACTED]} near-perfect success rate ($R^2>0.99$, $n=74$,
zero catastrophic failures) appeared to confirm this. \textbf{This has
since been corrected}: \textbf{[VALUE REDACTED --- pending re-verification against the pipeline's actual output; see reconciliation notes]}
HypatiaX leads $74.6\%$ vs.\ $60.5\%$ (see the corrected observation in
Section~\ref{sec:leaderboard} above). The routing fixes still combine the
LLM's structural extrapolation accuracy with the NN's calibration
correction as designed; what changed since the bug fix is the exact
margin by which the hybrid leads, not the direction of the comparison.
...
\section{Complete Proof of the Main Paper's \emph{LLM Reproductive Behaviour} Finding}
\label{app:proof}
...
# [Corrected: previously cited run_comparative_suite_benchmark_v2.py with
# --threshold 0.9999 and --pysr-timeout 900. That command/threshold does
# not match the verified released source file for this run; see the
# supplementary benchmark report's Phase-3 reproduction-command
# correction. Regenerated to match the verified command below.]
python hypatiax/experiments/benchmarks/\
run_protocol_benchmark_core.py \
    --noiseless --threshold 0.999999 \
    --nn-seeds 3 --samples 200 \
    --method-timeout 900 --pysr-timeout 1100
...
Seaborn      & 0.13.2  & \\
scikit-learn & 1.7.2   & \\
Anthropic SDK & 0.73.0 & \texttt{claude-sonnet-5}, temp~0.25 \textbf{[Corrected:
  previously listed as \texttt{claude-sonnet-4-20250514}, temp~0.0 --
  stale copy; \texttt{hypatia.py} hardcodes \texttt{claude-sonnet-5}/temp~0.25,
  confirmed the value actually used, since \texttt{LLM\_MODEL} and
  \texttt{repro.yaml}'s \texttt{llm\_model} field are dead code that
  nothing reads. See Issue 11.]} \\
\midrule
...
Hybrid LLM+NN (v50\_2)     & 30  & 45 min \\
...
\item LLM calls: temperature \texttt{0.25} \textbf{[Corrected: previously
  listed as \texttt{0.0} (deterministic) for all methods -- stale copy;
  see Issue 11]} for all methods
...
See the \emph{Feynman Benchmark Reproducibility Statement} appendix and the
\emph{Measurement Correction (v1$\to$v2)} section of Supplementary~B
(\emph{HypatiaX: Noise Robustness, Sample Complexity, and Convergence of
Hybrid LLM--Neural Symbolic Regression on the Feynman Benchmark}) for the
exact code change and affected equations.
...
\item Run DeFi benchmark --- all HypatiaX near-perfect-rate and Mean $R^2$
  figures for this benchmark are \textbf{[VALUES REDACTED]} pending
  re-verification against the pipeline's actual output; see main paper's
  Hybrid Decision-Attribution Bug section and Section~\ref{sec:leaderboard}
  above)
...
\textbf{[Superseded --- do not cite this subsection's absolute timings or
speedup figures without independent reproduction.]} The profiling below
predates the mechanical, script-driven regeneration of the runtime table
directly from raw per-task result files across the complete
5-seed$\times$74-task$\times$2-split sweep (main paper
\S\,10.5, ``Runtime Analysis'', Table~5). That regeneration
shows the opposite pattern from what follows: the hybrid system is
$2.5$--$9.3\times$ \emph{slower} than the neural baseline on every seed
and every split, never faster. In particular, the $1.73\times$ speedup
and the $1.7$\,s neural-network timing quoted below do not reproduce;
raw per-task neural-network times in the current benchmark cluster around
$0.34$--$0.42$\,s, roughly $4\times$ smaller than the $1.7$\,s figure
below, and the discrepancy is large enough to suggest this profiling
reflects a different, earlier run configuration rather than a computation
error. The scalability table (\S\,\ref{tab:scalability}, $5.1$--$5.8\times$)
and the parallelisation projections that follow are built on the same
unreproduced baseline and should be read with the same caution. Retained
here for historical/methodological interest only.
...
\item Neural networks are 27$\times$ faster on this timing benchmark. \textbf{[Correction, F-1: the 1231\% extrapolation-error figure previously cited here could not be reproduced from any data source currently available for this paper; see the main paper's regenerated Five-System Comparison tables (Section~5, ``Five-System Comparative Analysis'') for the current, verified extrapolation-error figures instead.]}
\end{itemize}
...
\begin{table}[H]
\centering
\caption{Runtime vs.\ Interpolation Performance Tradeoff. \textbf{[Correction,
  F-1: this table previously included an ``Extrapolation Error'' column
  reporting Neural Network = 1231\%, LLM-Guided Hybrid = 0.0\%, Pure PySR =
  0.0\%. Those figures could not be reproduced from any data source
  currently available for this paper and have been removed, along with the
  cost-per-point-reduced and $\infty$ rows that were derived from them. See
  the main paper's regenerated Five-System Comparison tables (Section~5)
  for the current, verified extrapolation-error figures, which do not
  support a near-zero characterisation for the hybrid system.]}}
\label{tab:cost_accuracy_tradeoff}
\begin{tabular}{lcc}
\toprule
\textbf{Method} & \textbf{Runtime (s)} & \textbf{Interpolation $R^2$} \\
\midrule
Neural Network & 1.7 & 0.940 \\
LLM-Guided Hybrid & 70.0 & 0.931 \\
Pure PySR & 390.0 & 0.982 \\
\bottomrule
\end{tabular}
\end{table}

\paragraph{Interpretation:} \textbf{[Correction, F-1: this paragraph
previously claimed the $41\times$ runtime increase (1.7s $\to$ 70s) reduced
mean extrapolation error from 1{,}231\% to near-zero, and that Pure PySR
gave no additional benefit over Hybrid on extrapolation. Those
extrapolation-error figures do not reproduce from any available data
source; see the main paper's regenerated Five-System Comparison tables
(Section~5) for the current figures. What remains supported by this
table is the runtime/interpolation tradeoff itself: Hybrid trades a
$41\times$ runtime increase over the neural baseline for a modest
interpolation $R^2$ gain (0.940 $\to$ 0.931 is actually a slight decrease
on this particular sweep), and Pure PySR's further $5.6\times$ runtime
increase over Hybrid yields a small interpolation $R^2$ gain (0.931 $\to$
0.982). No extrapolation claim is made here pending resolution of the
figures above.]}

\subsection{Parallelisation and Future Speedup}

PySR was configured with 2 parallel populations on the experimental hardware (Intel Celeron T3100, 2-core CPU). On modern multi-core hardware (e.g.\ 16+ cores), PySR can exploit 12 or more parallel populations, yielding an estimated ${\sim}8\times$ wall-clock speedup relative to our reported runtimes. GPU parallelisation of fitness evaluation (75\% of runtime) could further reduce total runtime to ${\sim}1$--$2$ seconds per test, making the hybrid approach competitive with neural networks on speed. \textbf{[Correction, F-1: the original clause here (``while retaining near-zero extrapolation error'') has been removed; see the main paper's regenerated Five-System Comparison tables (Section~5) for the current extrapolation-error figures, which are not near-zero for the hybrid system.]}
...
Our empirical results (95\% classical vs 40\% DeFi, see the Pure LLM Baseline table in Supplementary~B) raise a question: \textbf{Why does LLM performance decline on specialised domains?}\citep{kambhampati2024llms}
...
The central lesson of this work is that \textbf{retrieval of familiar patterns may differ from structural discovery}\citep{kambhampati2024llms}. On the evaluated benchmark, LLMs achieved high success rates on widely-documented equations (approximately 95\% on classical physics) while showing substantially lower reliability in specialized domains requiring genuine structural inference (approximately 40\% on DeFi; extrapolation error statistics in the main paper's Five-System Comparison table).

This distinction matters for science. Discovery requires:
\begin{enumerate}
\item Extrapolation to novel regimes (see the main paper's regenerated Five-System Comparison tables, Section~5, for the current per-system figures; an earlier ``symbolic near-zero vs.\ substantially larger neural error'' characterisation here did not reproduce and has been removed)
...
% =============================================================================
% INSERTED FROM jmlr_paper_main.tex — Appendix B & C
% =============================================================================
