...
noiseless protocol.
\textbf{[Correction, Phase~3 audit: a prior version of this abstract attributed
\HDS's 90.0\% (27/30) figure to \HSL. This has been corrected here; see the
companion \emph{Phase 3 Reproducibility Audit: Consolidated Findings and Bug
Recovery History} report, Item~2a.]}
\textbf{[Further correction: the previously printed 90.0\% (27/30), then
80.0\% (24/30), then 66.7\% (20/30) figures did not match any verified source
file. \texttt{protocol\_core\_noiseless\_20260812\_203510.json} does not
exist in the released artifacts, and no ten-shard file set exists either.
This document's noiseless source is
\texttt{protocol\_core\_noiseless\_20260812\_170344.json} (the only verified
noiseless file supplied), a 30-equation custom benchmark spanning 11 domains
...
which is \textbf{not} the Feynman-30 suite and includes no mathematics or
economics domain. The real \HDS{} figure on this verified file is 70.0\%
(21/30). See tablenote on \Cref{tab:overall}.]}
\textbf{[Third correction, this pass: the 27/30 (90.0\%) figure itself has
now been traced. It belongs to neither \HSL{} (30/30 on this file) nor
\HDS{} (21/30, confirmed above) --- it exactly matches \SEL{}
(\texttt{SymbolicEngineWithLLM (tools)}), independently recomputed from
\texttt{protocol\_core\_noiseless\_20260812\_170344.json} at the same
$\Rsq \ge 0.999999$ threshold as the other two figures: 27/30 (90.0\%),
matching to the equation. The abstract's repeated 27/30 citation is
therefore a persistent method-mislabeling across three correction passes
(first attributed to \HDS{}, now identified as \SEL{}), not a data error;
no table value changes as a result of this correction, only the
attribution.]}
...
split: HypatiaX achieves 90.5\,\% near-perfect success, eliminating all 6
...
(Phase~3 audit, Item~10b, closed):} the fix (a thread-safe \texttt{\_ProcBox}
...
wall-clock time rather than only the abandoned thread's timeout. See the
...
Anthropic SDK    & 0.73.0 (\texttt{claude-sonnet-5},
                   ...
                   as \texttt{claude-sonnet-4-20250514}, temperature 0.0
                   -- stale copy; \texttt{hypatia.py} hardcodes
                   \texttt{claude-sonnet-5}/temp~0.25, confirmed the
                   value actually used, since \texttt{LLM\_MODEL} and
                   \texttt{repro.yaml}'s \texttt{llm\_model} field are
                   dead code that nothing reads. See Issue 11.]} \\
\midrule
...
& Model & claude-sonnet-5 \textbf{[Corrected: was
  claude-sonnet-4-20250514, a stale copy; see Issue 11]} \\
& Temperature & 0.25 \textbf{[Corrected: was listed as 0.0
  (deterministic); \texttt{hypatia.py} hardcodes temperature~0.25]} \\
& Max tokens & 1024 \\
& Top-p & 1.0 \\
