...
class SymbolicEngine:
    ...
    def discover(
        ...
        **kwargs,
        ...
        # ------------------------------------------------------------------
        # ENHANCED PREPROCESSING + FEATURE ENGINEERING (TRACE-SAFE)
        # ------------------------------------------------------------------
        ...
        y = np.where(np.abs(y) < 1e-12, 1e-12, y)

        _trace.append("stability=applied")

        # --- FEATURE AUGMENTATION ---
        def _augment_features(X):
            feats = [X]

            # inverse (critical for rational equations)
            feats.append(1 / (X + 1e-8))
        ...
        X_aug = _augment_features(X)

        _trace.append(f"features={X.shape[1]}→{X_aug.shape[1]}")

        print(f"   Features: {X.shape[1]} → {X_aug.shape[1]}")

        # --- ROBUST OPERATOR SET ---
        active_unary = list(set(
        ...
        _trace.append(f"unary_ops={sorted(active_unary)}")

        # --- DATA-DRIVEN EXP DETECTION ---
        try:
            if np.all(y > 0):
                from sklearn.linear_model import LinearRegression
                from sklearn.metrics import r2_score

                r2_lin = r2_score(y, LinearRegression().fit(X, y).predict(X))
                logy = np.log(y)
                r2_log = r2_score(logy, LinearRegression().fit(X, logy).predict(X))

                if r2_log > r2_lin + 0.05:
                    if "exp" not in active_unary:
                        active_unary.append("exp")
                    print("   [AUTO-EXP DETECTED]")
                    _trace.append("auto_exp=1")
        except Exception:
            _trace.append("auto_exp=fail")

        # --- LOG-SENSITIVE DETECTION (HENDERSON FIX) ---
        _eq_hint = (equation_name or "").lower()

        if any(k in _eq_hint for k in ["henderson", "ph", "acid", "base", "log"]):
            if "log10" not in active_unary:
                active_unary.append("log10")
            print("   [FORCED LOG MODE]")
            _trace.append("forced_log=1")

        # --- EARLY FALLBACK (DIMENSION CONTROL) ---
        if X_aug.shape[1] > 15:
            print("   [EARLY FALLBACK] Feature explosion → SymbolicTreeEngine")
            _trace.append("fallback=early_tree")

            try:
                _te = SymbolicTreeEngine(max_depth=4, population_size=300, iterations=40)
                _tr = _te.discover_validate_interpret(X, y, variable_names=safe_names)
                ...
                return {
                    ...
                    "predictions": np.zeros_like(y),
                    "validation": {"valid": True, "errors": [], "warnings": []},
                    "method": "SymbolicTreeEngine (early fallback)",
                    "trace": _trace,
                }

            except Exception as _ef:
                print(f"   [EARLY FALLBACK FAILED] {_ef}")
                _trace.append("fallback=failed")

        # --- STRONGER SEARCH SETTINGS ---
        self.config.parsimony = max(self.config.parsimony, 0.02)
        self.config.maxsize = max(self.config.maxsize, 40)

        _trace.append(f"parsimony={self.config.parsimony}")
        _trace.append(f"maxsize={self.config.maxsize}")

        print(f"   Operators: {sorted(active_unary)}")
        ...
        try:
            ...
            # ------------------------------------------------------------------
            # ADAPTIVE TIMEOUT SAFETY
            # ------------------------------------------------------------------

            # single-variable problems should never run for many minutes
            if X.shape[1] == 1:
                _timeout = min(_timeout, 120)

            # augmented feature spaces should remain bounded
            if X_aug.shape[1] > 8:
                _timeout = min(_timeout, 180)

            # poor previous score → fail fast
            if _prior_score is not None and _prior_score < 0.25:
                _timeout = min(_timeout, 90)

            _trace.append(f"adaptive_timeout={_timeout}")
            ...
            def _fit_worker_process(queue, X, y, names, kwargs):
                ...
                try:
                    ...
                    # ------------------------------------------------------------------
                    # HARD GUARDRAILS (INSIDE TRY - CORRECT LOCATION)
                    # ------------------------------------------------------------------
                    
                    if X.shape[1] == 1:
                        print("   [FAST PATH] Single variable → reduced search")
                        self.config.niterations = min(self.config.niterations, 40)
                        self.config.population_size = min(self.config.population_size, 50)
                        self.config.populations = min(self.config.populations, 5)
                        _trace.append("fastpath=1var")
                    if np.std(y) < 1e-6:
                        print("   [SKIP] Near-constant target")
                        _trace.append("skip=constant_y")
                        return {
                            "success": True,
                    ...
                    if _prior_score is not None and _prior_score < 0.25:
                        print("   [FORCED FALLBACK] score too low → tree engine")
                        _trace.append("forced_fallback=low_score")
                        ...
                        return {
                            ...
                            "original_variable_names": variable_names,
                            "variable_name_mapping": name_mapping,
                            "method": "SymbolicTreeEngine (forced)",
                            "trace": _trace,
                        }

                    # ------------------------------------------------------------------
                    # PySR MODEL STARTS HERE (UNCHANGED BELOW)
