"""
HypatiaX Hybrid Discovery System v4.1
======================================
Bug-fix release on top of v4.0 (reproducibility-pinned v5.0 variant).
...
FIX-B   extrapolation_errors always 0.0 / extrap_r2 always wrong.
        Root cause: the discover() thin adapter returned the full expression
        string in "final_formula" but test harnesses computing out-of-
        distribution metrics looked for a "formula" key in the result dict.
        Fix: return both "formula" and "final_formula" pointing to the same
        expression string, and add a "variable_names" key so callers can
...
FIX-C   PySR non-determinism warning on every run.
        ...
        parallelism='serial' into DiscoveryConfig defaults when a
        random_state is supplied (opt-out via allow_nondeterministic=True).
...
FIX-D   Gravitational-force (and other extreme-scale equations) collapses
        ...
        any feature), applies per-feature log10 scaling with sign handling,
        fits on the scaled space, and injects a scale_log=True flag into the
        result metadata so callers know the returned expression is in log-
        space.  Raw RMSE is still computed in original units.

Inherited from v4.0
-------------------
PIN-1…4  Reproducibility pins (max_retries=5, niterations=50, warm-start
         disabled, physics_fallback=False).
FIX-1…6  v5.0 LLM-wiring fixes.
PROD-1…7 v4.1-PROD performance improvements.
"""
...
class HybridDiscoverySystem:
    """
    Hybrid discovery system v4.0 (reproducibility-pinned).
    ...
    def __init__(
        ...
        enable_physics_fallback: bool = False,  # PIN-4: off by default (not used in v4 ref run)
        ...
        """
        Initialize HybridDiscoverySystem v4.1.
        ...
        allow_nondeterministic : bool
            ...
            'serial' on the DiscoveryConfig so PySR suppresses its non-
            determinism warning and results are reproducible across runs.
        ...
        logger.info("=" * 70)
        logger.info("HybridDiscoverySystem v4.1 — BUG-FIX RELEASE")
        logger.info("=" * 70)
        ...
        # FIX-C: Enforce deterministic PySR unless caller explicitly opts out.
        # PySR emits a warning on every run when random_state is set without
        # deterministic=True + parallelism='serial'.  Setting these here once
        # suppresses the warning and makes searches reproducible.
        if not allow_nondeterministic:
            if hasattr(symbolic_config, "deterministic"):
                symbolic_config.deterministic = True
            if hasattr(symbolic_config, "parallelism"):
                symbolic_config.parallelism = "serial"
            # Store as instance attrs so _discover_with_retry can pass them
            # directly to PySR via pysr_kwargs if the engine supports it.
            self._pysr_deterministic = True
            self._pysr_parallelism = "serial"
        else:
            self._pysr_deterministic = False
            self._pysr_parallelism = None
        logger.info(
            f"[PySR] deterministic={self._pysr_deterministic}, "
            f"parallelism={self._pysr_parallelism!r}"
        )
        ...
        logger.info("[OK] HybridDiscoverySystem v4.1 initialized\n")
    ...
    def _inject_operators(symbolic_config: DiscoveryConfig, domain: str) -> None:
        ...
        if _needs_inv_trig:
            ...
            if not _current:
                ...
                logger.info(
                    f"[AUTO-v4.1] unary_operators was empty — seeding with trig defaults: {_current}"
                )
            _added = [op for op in _inv_trig if op not in _current]
            if _added:
                symbolic_config.unary_operators = _current + _added
                logger.info(
                    f"[AUTO-v4.1] Injected inverse-trig operators {_added} "
                    f"(use_tc=True). Full unary set: {symbolic_config.unary_operators}"
                )
        else:
            logger.info(
                f"[AUTO-v4.1] Skipping safe_asin/safe_acos injection "
    ...
    def discover_validate_interpret(
        ...
        """
        Complete discovery workflow v4.1.
        ...
        try:
            ...
            # symbolic_engine.py).
            _WS_THRESHOLD = -1.0  # PIN-3: warm-start disabled (set > 0 to re-enable)
    ...
    def discover(
        ...
        # constant (e.g. gravitational force with 1e-9 charges and 1e22 output).
        # Apply signed log10 scaling per feature and flag the result so callers
        # know the returned expression lives in log-space.
        _log_scaled = False
        _X_orig = X
        _y_orig = y
        _log_scale_threshold = 6  # log10 orders of magnitude

        def _safe_log_scale(arr: np.ndarray) -> np.ndarray:
            """Signed log10: sign(x) * log10(|x| + 1).  Safe for zeros."""
        ...
        try:
            ...
            for col_idx in range(X.shape[1]):
                ...
                if len(abs_finite) < 2:
                    _needs_log.append(False)
                    continue
                _range = np.log10(np.max(abs_finite)) - np.log10(np.min(abs_finite))
                _needs_log.append(_range > _log_scale_threshold)

            # Also check the target y
            _y_abs = np.abs(y[np.isfinite(y) & (y != 0)])
            _y_needs_log = (
                len(_y_abs) >= 2
                and (np.log10(np.max(_y_abs)) - np.log10(np.min(_y_abs))) > _log_scale_threshold
            )

            if any(_needs_log) or _y_needs_log:
                _log_scaled = True
                X_scaled = X.copy().astype(float)
                for col_idx, do_log in enumerate(_needs_log):
                    if do_log:
                        X_scaled[:, col_idx] = _safe_log_scale(X[:, col_idx])
                        logger.info(
                            f"[FIX-D] Feature '{var_names[col_idx]}' log10-scaled "
                            f"(range > {_log_scale_threshold} orders of magnitude)"
                        )
                y_fit = _safe_log_scale(y) if _y_needs_log else y
                if _y_needs_log:
        ...
        try:
            ...
            # ── FIX-A: compute RMSE from the expression, not a missing key ────
            # Previous versions looked for discovery["predictions"] which was
            # never written, so rmse was always float("inf").  We now evaluate
            # the expression string directly against the *original* (unscaled) y.
            ...
            if formula and formula not in (
                ...
                try:
                    # Build variable bindings from original (unscaled) X
                    _test_data = {
                        name: _X_orig[:, i] for i, name in enumerate(var_names)
                    }
                    # Use the same normalise + eval path as _safe_validate
                    _norm_expr = self._normalise_expression(formula)
                    # Evaluate expression in a restricted namespace
                    _ns: Dict[str, Any] = {"np": np, **_test_data}
                    _y_pred = eval(_norm_expr, {"__builtins__": {}}, _ns)  # noqa: S307
                    _y_pred = np.asarray(_y_pred, dtype=float)
                    if _y_pred.shape == ():
                        _y_pred = np.full(len(_y_orig), float(_y_pred))
                    _finite = np.isfinite(_y_pred) & np.isfinite(_y_orig)
                    if _finite.sum() >= 2:
                        rmse = float(
                            np.sqrt(np.mean((_y_orig[_finite] - _y_pred[_finite]) ** 2))
                        )
            ...
            # ── FIX-B: expose all keys test harnesses look for ────────────────
            # Harnesses extract the expression via "formula", "expression", or
            # "final_formula" — all three must point to the same string.
            # "variable_names" lets callers bind the expression without an extra
            # round-trip into the full_result structure.
            return {
                "success": success,
                "r2": r2,
                "rmse": rmse,
                # FIX-B: all three aliases for the expression string
                "formula": formula,
                "expression": formula,
                "final_formula": formula,
                # FIX-B: variable names so harnesses can bind the expression
                "variable_names": var_names,
                ...
                # FIX-D: flag whether the expression is in log-space
                "scale_log": _log_scaled,
                "error": None,
            }
    ...
    def save_results(self, filename: Optional[str] = None) -> str:
        ...
        if filename is None:
            ...
            filename = f"discovery_results_v41_{timestamp}.json"
...
# ============================================================================
# QUICK TEST  — covers all four v4.1 bug fixes
# ============================================================================

if __name__ == "__main__":
    print("=" * 80)
    print("HYBRID SYSTEM v4.1 — QUICK TEST")
    print("=" * 80)

    # ── Test A: pure PySR, Ohm's Law — verifies FIX-A (RMSE) + FIX-B (keys)
    print("\nTest A: Ohm's Law — pure PySR (FIX-A: RMSE, FIX-B: keys)")
    print("-" * 80)
    np.random.seed(42)
    ...
    res_a = sys_a.discover(
        ...
        metadata={"equation_name": "ohms_law",
                  "variable_descriptions": {"I": "Current", "R": "Resistance"},
                  "variable_units": {"I": "A", "R": "Ohm"}},
    )
    print(f"  formula       : {res_a['formula']}")
    print(f"  final_formula : {res_a['final_formula']}")   # FIX-B: alias present
    print(f"  expression    : {res_a['expression']}")      # FIX-B: alias present
    ...
    print(f"  scale_log     : {res_a['scale_log']}")       # FIX-D: should be False
    sys_a.print_statistics_summary()

    # ── Test B: Coulomb / gravitational — FIX-D (extreme-scale log transform)
    print("\nTest B: Gravitational Force — extreme-scale log transform (FIX-D)")
    print("-" * 80)
    np.random.seed(42)
    G = 6.674e-11
    m1 = np.random.uniform(1e-9, 1e-6, 120)
    m2 = np.random.uniform(1e-9, 1e-6, 120)
    r  = np.random.uniform(0.01, 1.0,  120)
    F  = G * m1 * m2 / r**2
    ...
    res_d = sys_d.discover(
        ...
        metadata={"equation_name": "gravity",
                  "variable_descriptions": {"m1": "mass 1 kg", "m2": "mass 2 kg", "r": "distance m"},
                  "variable_units": {"m1": "kg", "m2": "kg", "r": "m"}},
    )
    print(f"  formula   : {res_d['formula']}")
    print(f"  R²        : {res_d['r2']:.4f}")
    print(f"  RMSE      : {res_d['rmse']}")
    log_applied = res_d['scale_log']
    print(f"  scale_log : {log_applied}  {'[FIX-D: log-transform applied]' if log_applied else '[no extreme scale detected]'}")
    print(f"  success   : {res_d['success']}")

    # ── Test C: deterministic mode — FIX-C (no PySR non-determinism warning)
    print("\nTest C: Deterministic PySR — should produce no non-determinism warning (FIX-C)")
    print("-" * 80)
    sys_c = HybridDiscoverySystem(
        domain="physics",
        discovery_config=DiscoveryConfig(niterations=20, enable_auto_configuration=False),
        enable_physics_fallback=False,
        max_retries=2,
        use_llm=False,
        allow_nondeterministic=False,   # FIX-C: default, but explicit here
    )
    print(f"  _pysr_deterministic : {sys_c._pysr_deterministic}")
    print(f"  _pysr_parallelism   : {sys_c._pysr_parallelism!r}")
    print("  (run above — if no 'non-deterministic searches' UserWarning appeared, FIX-C OK)")

    # ── Test D: LLM hybrid mode (optional, requires ANTHROPIC_API_KEY)
    print("\nTest D: LLM hybrid mode (use_llm=True, requires ANTHROPIC_API_KEY)")
    print("-" * 80)
