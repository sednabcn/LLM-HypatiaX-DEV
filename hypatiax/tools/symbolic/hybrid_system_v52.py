        ...
        **kwargs,
        ...
        try:
                    ...
                    if _y_fit_pos:
                        ...
                        if _looks_like_exp and not _looks_like_powerlaw:
                            ...
                            print(
                                ...
                                f"R²(log y~log X)={_r2_loglog:.3f} — exponential detected, injecting exp/log",
                                flush=True,
                            )
                        elif _looks_like_exp and _looks_like_powerlaw:
                            print(
                                f"   [AUTO-EXP] data heuristic: R²(log y~X)={_r2_log:.3f}, "
                                f"R²(log y~log X)={_r2_loglog:.3f} — power law detected, "
                                f"skipping exp/log injection (log-log fit dominates)",
                                flush=True,
                            )
                except Exception:
            ...
            if hasattr(self.model, "equations_") and len(self.model.equations_) > 0:
                ...
                print(f"   ✅ Found: {_expression_display if _x_scaled_cols else expression}")
...
print("=" * 80)
print("HYBRID SYSTEM v5.2 — QUICK TEST (covers FIX-A … FIX-D)")
print("=" * 80)
