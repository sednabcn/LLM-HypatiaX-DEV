...
def pca_directed_split(X, y, test_size=0.6, random_state=None):
    """
    Perform a PCA-directed train/test split.
    ...
    # Normalise inputs to numpy/pandas types we can index uniformly
    X_df = X.copy() if isinstance(X, pd.DataFrame) else pd.DataFrame(X)
    y_series = y.copy() if isinstance(y, pd.Series) else pd.Series(y, name="target")

    n_samples, n_features = X_df.shape
    if n_samples == 0 or n_features == 0:
        raise ValueError("Cannot perform PCA on an array with zero samples or features.")

    # --- 1. Project onto first principal component ---
    n_components = min(n_samples, n_features, 1)
    pca = PCA(n_components=n_components, random_state=random_state)
    pc1_scores = pca.fit_transform(X_df)[:, 0]

    # --- 2. Sort by PC1 score ---
    order = pd.Series(pc1_scores, index=X_df.index).sort_values().index

    # --- 3. Split at the (1 - test_size) quantile ---
