...
Step IDs (use with --only / --from):
    ...
    Phase 1 : exp1  exp1b  exp2_feynman  exp2_sym  exp2_hyb  exp2  exp2b  exp3  exp3b
...
def run_exp2_feynman(env: dict, args, log_fh) -> bool:
    """
    DEAD CODE — inline_runner=True is no longer set on any Step.

    This function is retained for reference only.  The exp2_feynman step
    (inline_runner=False) now delegates to experiment_protocol_feynman_exp2.py
    via a standard Popen subprocess, which runs the canonical protocol-layer
    entry point (→ run_protocol → run_task → exp2_feynman_colab_multithreaded.py).

    The 6-method Feynman-30 *comparison* suite is handled by the exp2b step
    (run_comparative_suite_benchmark_v2.py --protocol feynman), not here.
...
STEPS: list[Step] = [
    ...
    # §10.7 Feynman-30: 6-method comparison suite on the 30 Feynman equations.
    # Mirrors exp2 (which runs the multi-domain 30 via run_comparative_suite_benchmark_injected.py)
    # but uses run_comparative_suite_benchmark_v2.py --protocol feynman so that the same
    # six methods (PureLLM, NN, HybridDeFi, HybridAllDomains, SymbolicEngine, HybridV50_2)
    # are evaluated on the Feynman 30 equations and produce the companion comparison table.
    #
    # NOTE: exp2_feynman (above) runs experiment_protocol_feynman_exp2.py — the
    # HypatiaX-vs-NN *extrapolation* benchmark.  exp2b is a separate, orthogonal step:
    # it runs the full 6-method *comparison* suite on the same Feynman 30 equations.
    #
    # Resume:
    #   python3 run_all_checkpoint.py --resume
    #   → exp2b skips if logs/pipeline_checkpoint.json marks it pass
    #
    # Run individually:
    #   python3 hypatiax/experiments/benchmarks/run_comparative_suite_benchmark_v2.py \
    #           --protocol feynman --resume --samples 200
    Step("exp2b",
         "Exp 2b · Feynman-30 6-method comparison suite  (§10.7)",
         [sys.executable,
