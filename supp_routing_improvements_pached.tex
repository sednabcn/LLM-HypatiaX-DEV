...
\caption{Baseline performance before routing improvements (standard cases,
         ...
         success-attribution bug; the corrected figure is
         \textbf{60.8\%} ($45/74$), below Pure LLM's $62.2\%$. See that
         section before citing either number as current.}
\label{tab:baseline}
\begin{tabular}{lcccc}
\toprule
...
\textbf{v3.0 (bug-corrected)} & Decision-attribution bug removed & $-29.7$ & \textbf{60.8} & \textbf{measured} \\
\bottomrule
\end{tabular}
\end{table}
...
subsequently found to be inflated by a success-attribution bug: 22 of the
67 reported near-perfect successes credited a sub-method
(\texttt{hybrid.decision}) that the pipeline's own records show had
actually failed on that task. The corrected figure is $45/74 = 60.8\%$,
which is \emph{below} Pure LLM's $62.2\%$ on the same run --- the opposite
of the comparison this document originally reported. This does not
...
LLM by 28.3pp" framing based on $90.5\%$ should not be cited; see the main
...
\item Run DeFi benchmark --- confirm the corrected 60.8\% figure (not the
  superseded uncorrected 90.5\%), and that it sits below Pure LLM's
  62.2\% (n=74, v3.0; see main paper's Hybrid Decision-Attribution Bug
  section)
