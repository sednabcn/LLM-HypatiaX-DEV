...
\begin{itemize}
  \item \textbf{Section~\ref{sec:feynman_benchmark} Feynman Extrapolation Benchmark ($n=30$)} --- reports
  ...
  \item \textbf{Section~\ref{sec:nguyen12} Nguyen-12 Benchmark} --- new section reporting
...
\bibitem[Roberts et~al.(2023)]{contamination2023check}
...
\bibitem[Satvaty et~al.(2024)]{memorization2024audit}
...
All experimental results in Section~\ref{sec:benchmark} were produced
...
\paragraph{Phase~2 (noisy, $R^2>0.995$, Table~\ref{tab:feynman30}).}
\begin{verbatim}
...
\paragraph{Phase~3 (noiseless, $R^2>0.9999$, Tables~\ref{tab:feynman30}
and~\ref{tab:feynman30}).}
\begin{verbatim}
python run_comparative_suite_benchmark_v2.py \
...
Table~\ref{tab:changes} (Section~\ref{sec:r2_bugfix}) summarises all
...
from all scientific comparisons (Section~\ref{sec:results}).
...
Table~\ref{tab:nguyen} provides this comparison.
...
The Michaelis-Menten and Impermanent Loss equations were recovered with constant error $< 10^{-5}$, achieving $R^2 = 1.0000$ and near-zero extrapolation error (see Listing~\ref{sec:results}).
...
\begin{itemize}
\item \textbf{Interpolation}: $R^2 = 0.21$ (poor fit; relative error $> 100\%$, meeting the threshold in Definition~\ref{def:ood_extrap})
...
To assess correspondence with Condition 3 of Empirical Result~\ref{thm:llm_reproductive} (noise tolerance threshold $\sigma \leq 0.1 \cdot \text{std}(y)$), we conducted systematic experiments varying noise levels.
