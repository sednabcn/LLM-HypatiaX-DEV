...
\begin{enumerate}
  ...
  \item \textbf{[Open, process.]} The legacy exp2 baseline-lock threshold
    fix ($R^2 \geq 0.999999$ replacing $R^2 \geq 0.9999$, which had
    inflated 74/90 to 81/90) is applied in code but not yet confirmed
    against a live CI run; 81/90 should not be cited as current pending
    that confirmation.
\end{enumerate}
...
Process/CI credibility  & \multicolumn{2}{c}{\textcolor{warnAmber}{\textbf{OPEN}}} &
  Do not cite July~9 dashboard or 81/90 baseline until live-confirmed (\S\ref{suppb:limitations})\\
\midrule
\textbf{Overall verdict} & \textcolor{passgreen}{\textbf{READY}} & \textcolor{passgreen}{\textbf{READY}} &
  Ready on accuracy/robustness grounds; two process items above remain open\\
\bottomrule
\end{tabular}
\end{table}
