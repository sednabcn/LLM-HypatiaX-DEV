...
\caption{Baseline performance before routing improvements (standard cases,
         ...
         previously-reported \textbf{90.5\%} figure is now disclosed in the
         ...
         success-attribution bug.
         \textbf{[Correction: this note previously stated the corrected v3.0
         figure was 60.8\% (45/74), below Pure LLM's 62.2\%. That 60.8\%
         figure is Pure LLM's own near-perfect rate, not Hybrid's --- see
         the main paper's Hybrid Decision-Attribution Bug section for the
         reproduced numbers. Direct computation across all five available
         v3.0 seeds shows Hybrid beats Pure LLM by a wide, consistent
         margin (mean 74.6\% vs.\ 60.5\%, every seed). Do not cite 60.8\%
         as Hybrid's rate.]} See that section before citing any v3.0
         number as current.}
\label{tab:baseline}
\begin{tabular}{lcccc}
\toprule
...
\caption{Measured and projected $R^2 > 0.99$ (\%) by fix.
         ...
         v3.0 benchmark's uncorrected figure is \textbf{90.5\%}.
         \textbf{[Correction: this caption previously stated the main
         paper's decision-attribution-bug correction gives 60.8\% as
         the current v3.0 Hybrid figure. That 60.8\% is Pure LLM's own
         rate, not Hybrid's; see the main paper's Hybrid
         Decision-Attribution Bug section for the reproduced numbers
         (corrected Hybrid: 44/74, 59.5\%, on seed 42; Hybrid beats
         Pure LLM by a wide margin averaged across all five available
         seeds).]} See the corrected-observation note below the table.}
\label{tab:projected}
\begin{tabular}{llccc}
\toprule
...
\midrule
\textbf{v3.0 (uncorrected)} & All fixes + benchmark corrections & --- & \textbf{90.5} & \textbf{measured} \\
\textbf{v3.0 (bug-corrected)}\textsuperscript{$\dagger$} & Decision-attribution bug removed & $-30.9$ & \textbf{59.5} & \textbf{measured} \\
\bottomrule
\end{tabular}
\begin{flushleft}
\footnotesize\textsuperscript{$\dagger$}\,Seed~42, $\Rsq>0.99$ criterion, corrected per the main paper's
Hybrid Decision-Attribution Bug section (44/74). Pure LLM's own rate on
the same criterion/seed is 43/74 (58.1\%) --- Hybrid still edges it.
Averaged across all five available seeds at the standard $\Rsq>0.9$
near-perfect criterion (no attribution correction), Hybrid leads by a
much wider, highly consistent margin: mean 74.6\% vs.\ Pure LLM's
60.5\%, every seed.
\end{flushleft}
\end{table}
...
Bug'').} The $90.5\%$ figure reported for the $n=74$ v3.0 benchmark was
subsequently found to be inflated by a success-attribution bug.
\textbf{[Further correction: the ``22 of 67'' figure and the ``corrected
$45/74=60.8\%$, below Pure LLM's $62.2\%$'' comparison previously given
here do not reproduce from the source data. Reproducing the main paper's
stated methodology directly against
\texttt{hypatiax\_defi\_benchmark\_v3\_results\_seed42.json} finds 8 of
52 reported near-perfect successes ($\Rsq>0.99$) are attribution-fabricated,
giving a corrected rate of $44/74=59.5\%$ --- which is
\emph{above}, not below, Pure LLM's own rate on the same seed and
criterion ($43/74=58.1\%$). Averaged across all five available seeds
under the standard $\Rsq>0.9$ near-perfect criterion, Hybrid beats Pure
LLM by a wide, consistent margin (mean $74.6\%$ vs.\ $60.5\%$, every
seed, no exceptions). The headline ``Hybrid beats Pure LLM'' framing
is therefore not reversed by this correction --- it holds, just by a
smaller margin than the uncorrected $90.5\%$ figure implied.]} This
does not invalidate Fixes~0--5 individually (each addresses a distinct,
verified routing or data-generation defect); see the main paper for the
current, corrected comparison and its own caveats.
\end{observation}
...
figure of \textbf{90.5\%} near-perfect success rate ($R^2>0.99$, $n=74$,
zero catastrophic failures) appeared to confirm this. \textbf{This has
since been corrected}: the main paper's decision-attribution-bug fix
brings this down to $60.8\%$, below Pure LLM's $62.2\%$ on the same run
(see the corrected observation in Section~\ref{sec:leaderboard} above).
The routing fixes still combine the LLM's structural extrapolation
accuracy with the NN's calibration correction as designed; what changed
is which of the two methods currently has the higher confirmed
near-perfect rate.
...
\item Run DeFi benchmark --- confirm Hybrid's corrected near-perfect rate
  \textbf{[Corrected: previously said 60.8\%, below Pure LLM's 62.2\%.
  60.8\% is Pure LLM's own rate. Corrected Hybrid (seed 42,
  attribution-corrected, $\Rsq>0.99$): 44/74 = 59.5\%, above Pure LLM's
  43/74 = 58.1\% on the same seed/criterion. Averaged across all 5
  seeds at the standard $\Rsq>0.9$ criterion: Hybrid 74.6\% vs.\ Pure
  LLM 60.5\%, every seed. See main paper's Hybrid Decision-Attribution
  Bug section]}, not the superseded uncorrected 90.5\%
  (n=74, v3.0)
\item Run core-noiseless benchmark --- confirm EHD (Hybrid DeFi) 76.7\%
  (23/30, $R^2 \geq 0.999999$; \textbf{[Corrected: previously listed as
  96.7\%, then 80.0\%, tied to source files that do not exist or
  don't match; see Supplementary~B, tablenote on Table~tab:overall for
  the verified figure]})
...
\begin{enumerate}
\item Extrapolation to novel regimes (symbolic near-zero vs substantially larger neural error; see the main paper's Five-System Comparison table)
